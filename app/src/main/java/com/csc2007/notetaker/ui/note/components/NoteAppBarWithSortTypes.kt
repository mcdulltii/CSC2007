...
@Composable
fun NoteAppBarWithSortTypes(onEvent: (NoteEvent) -> Unit) {
    ...
    Row(
        ...
        Button(
            onClick = {
                onEvent(NoteEvent.SortNotes(if (isAscending) SortType.ASC_TITLE else SortType.DESC_TITLE))
                isTitle = true
            },
            modifier = buttonSize,
            border = commonBorder,
        ...
        Button(
            onClick = {
                onEvent(NoteEvent.SortNotes(if (isAscending) SortType.ASC_DATE_ADDED else SortType.DESC_DATE_ADDED))
                isTitle = false
            },
            modifier = buttonSize,
            border = commonBorder,
        ...
        Button(
            onClick = {
                onEvent(if (isTitle) NoteEvent.SortNotes(SortType.ASC_TITLE) else NoteEvent.SortNotes(SortType.ASC_DATE_ADDED))
                isAscending = true
            },
        ...
        Button(
            onClick = {
                onEvent(if (isTitle) NoteEvent.SortNotes(SortType.DESC_TITLE) else NoteEvent.SortNotes(SortType.DESC_DATE_ADDED))
                isAscending = false
            },
            modifier = Modifier.size(100.dp, 30.dp),
            border = commonBorder,
